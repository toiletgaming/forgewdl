...
public class WDLHooks {
	...
	/**
	 * Called when {@link WorldClient#tick()} is called. <br/>
					...
					if (WDL.thePlayer.openContainer != WDL.windowContainer) {
						if (WDL.thePlayer.openContainer == WDL.thePlayer.inventoryContainer) {
							...
							if (WDL.lastEntity != null) {
								...
								if (!handled) {
									WDLMessages.chatMessageTranslated(WDLMessageTypes.ON_GUI_CLOSED_WARNING,
								...
								if (!handled) {
									WDLMessages.chatMessageTranslated(WDLMessageTypes.ON_GUI_CLOSED_WARNING,
											"wdl.messages.onGuiClosedWarning.unhandledTileEntity", pos,
											sender.getTileEntity(pos));
								}
							}
	...
	public static void onWorldClientDoPreChunk(WorldClient sender, int x, int z, boolean loading) {
		try {
			...
			if (!loading) {
				...
				custom_ChunkRenderer.addChunk( sender.getChunk( x, z ) );
				LOGGER.debug("Adding new empty chunk at " + x + ", " + z + " (already has: "
						+ (sender.getChunkProvider().getLoadedChunk(x, z) != null) + ")");
			}
	...
	/**
	 * Called when {@link WorldClient#removeEntityFromWorld(int)} is called. <br/>
	 * Should be at the start of the method.
	 *
	...
	/**
	 * Called when {@link NetHandlerPlayClient#handleChat(SPacketChat)} is called.
	 * <br/>
	...
	public static void onNHPCHandleChat(NetHandlerPlayClient sender, SPacketChat packet) {
		try {
			...
			for (ModInfo<IChatMessageListener> info : WDLApi.getImplementingExtensions(IChatMessageListener.class)) {
				PROFILER.startSection(info.id);
	...
	/**
	 * Called when {@link NetHandlerPlayClient#handleMaps(SPacketMaps)} is called.
	 * <br/>
	 * Should be at the end of the method.
	 */
	...
	/**
	 * Called when
	 * {@link NetHandlerPlayClient#handleCustomPayload(SPacketCustomPayload)} is
	 * called. <br/>
	 * Should be at the end of the method.
	 */
	...
	/**
	 * Called when
	 * {@link NetHandlerPlayClient#handleBlockAction(SPacketBlockAction)} is called.
	 * <br/>
	 * Should be at the end of the method.
	 */
	public static void onNHPCHandleBlockAction(NetHandlerPlayClient sender, SPacketBlockAction packet) {
		try {
			...
			for (ModInfo<IBlockEventListener> info : WDLApi.getImplementingExtensions(IBlockEventListener.class)) {
				PROFILER.startSection(info.id);
	...
	/**
	 * Start button ID. Ascii-encoded 'WDLs' (World Downloader Start). Chosen to be
	 * unique.
	 */
	private static final int WDLs = ('W' << 24) | ('D' << 16) | ('L' << 8) | ('s');
	/**
	 * Options button ID. Ascii-encoded 'WDLo' (World Downloader Options). Chosen to
	 * be unique.
	 */
	...
	public static void injectWDLButtons(GuiIngameMenu gui, List buttonList) {
		...
		GuiButton wdlOptions = new GuiButton(WDLo, gui.width / 2 + 71, insertAtYPos, 28, 20,
				I18n.format("wdl.gui.ingameMenu.settings"));

		if (WDL.minecraft.isSingleplayer()) {
			wdlDownload.displayString = I18n.format("wdl.gui.ingameMenu.downloadStatus.singlePlayer");
			...
			if (WDLPluginChannels.canRequestPermissions()) {
				...
				wdlDownload.displayString = I18n.format("wdl.gui.ingameMenu.downloadStatus.request");
			} else {
				// Out of date plugin :/
				wdlDownload.displayString = I18n.format("wdl.gui.ingameMenu.downloadStatus.disabled");
				wdlDownload.enabled = false;
			}
		} else if (WDL.saving) {
			wdlDownload.displayString = I18n.format("wdl.gui.ingameMenu.downloadStatus.saving");
			wdlDownload.enabled = false;
			wdlOptions.enabled = false;
		} else if (WDL.downloading) {
			wdlDownload.displayString = I18n.format("wdl.gui.ingameMenu.downloadStatus.stop");
		} else {
			wdlDownload.displayString = I18n.format("wdl.gui.ingameMenu.downloadStatus.start");
		}
		...
		if (button.id == WDLs) { // "Start/Stop Download"
			...
			if (WDL.downloading) {
				...
				if (!WDLPluginChannels.canDownloadAtAll()) {
					...
					return;
				} else if (WDLPluginChannels.hasChunkOverrides() && !WDLPluginChannels.canDownloadInGeneral()) {
